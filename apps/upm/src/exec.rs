...
use crate::toolchain::{Toolchain, ToolchainKind};
...
fn new_makepkg<S: AsRef<OsStr>>(
    ...
    let toolchain = Toolchain::detect(dir);
    new_makepkg_with_toolchain(config, dir, args, pkgdest, &toolchain)
}

fn new_makepkg_with_toolchain<S: AsRef<OsStr>>(
    config: &Config,
    dir: &Path,
    args: &[S],
    pkgdest: Option<&str>,
    ...
    cmd.envs(toolchain.environment());
    cmd.env("UPM_SELECTED_TOOLCHAIN", toolchain.name());
...
pub fn makepkg_dest<S: AsRef<OsStr>>(
    ...
    let selected = Toolchain::detect(dir);
    let mut cmd = new_makepkg_with_toolchain(config, dir, args, pkgdest, &selected);
    let status = command_status(&mut cmd)?;
    let automatic = std::env::var("UPM_TOOLCHAIN").map_or(true, |value| value == "auto");
    if status.0 != 0 && selected.kind == ToolchainKind::Llvm && automatic {
        if let Some(fallback) = Toolchain::gcc_fallback() {
            eprintln!(
                "upm: LLVM build failed; retrying a clean build with GCC (set UPM_TOOLCHAIN=llvm to disable fallback)"
            );
            let mut cmd = new_makepkg_with_toolchain(config, dir, args, pkgdest, &fallback);
            cmd.arg("-C");
            return command_status(&mut cmd);
        }
    }
    Ok(status)
}
...
mod tests {
    ...
    #[test]
    fn retries_failed_llvm_makepkg_with_gcc() {
        let dir = tempfile::tempdir().unwrap();
        if Toolchain::detect(dir.path()).kind != ToolchainKind::Llvm {
            return;
        }

        let log = dir.path().join("toolchains.log");
        let makepkg = dir.path().join("makepkg");
        fs::write(
            &makepkg,
            format!(
                "#!/bin/sh\necho \"$UPM_SELECTED_TOOLCHAIN $*\" >> '{}'\n[ \"$UPM_SELECTED_TOOLCHAIN\" = gcc ]\n",
                log.display()
            ),
        )
        .unwrap();
        fs::set_permissions(&makepkg, fs::Permissions::from_mode(0o755)).unwrap();

        let mut config = Config::default();
        config.makepkg_bin = makepkg.display().to_string();
        let status = makepkg_dest::<&str>(&config, dir.path(), &[], None).unwrap();
        assert_eq!(status.0, 0);
        assert_eq!(fs::read_to_string(log).unwrap(), "llvm \ngcc -C\n");
    }
}
