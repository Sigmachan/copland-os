...
fn debug_enabled() -> bool {
    env::var("UPM_DEBUG").as_deref().unwrap_or("0") != "0"
}

fn alpm_debug_enabled() -> bool {
    debug_enabled() && env::var("UPM_ALPM_DEBUG").is_ok_and(|v| v != "0")
}
...
pub async fn run<S: AsRef<str>>(args: &[S]) -> i32 {
    let args = args
        ...
        .collect::<Vec<_>>();
    let args = match compat::prepare(&args) {
        compat::Prepared::Args(args) => args,
        compat::Prepared::Exit(code) => return code,
    };
