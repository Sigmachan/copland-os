...
UPM has a couple of feature flags which you may want to enable:
...
UPM's test suite can be run by running:
...
See https://github.com/Morganamilo/upm/discussions/433 for discussion on localization.
...
To translate upm to a new language, copy the the template .pot file to the locale you
are translating to.

For example, to translate upm to Japanese you would do:

```
cp po/upm.pot po/jp.po
```
...
To test the translations you first must build the translation then run upm
