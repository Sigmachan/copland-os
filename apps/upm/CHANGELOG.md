...
## UPM v2.0.3 (2024-03-13)

### Fixed

- Fix build on arm c096bb

## UPM v2.0.2 (2024-03-13)
...
## UPM v2.0.0 (2023-11-26)

UPM v2.0.0 comes after a long time of no releases, mostly due to a lack of free time
...
engine. You can now add pkgbuild repos to your upm.conf in the form of:
...
Then syncing the repo with `upm -Sy --pkgbuilds`.

You can also specify 'Path =' instead to point to a pkgbuild repo on disk.

upm will then recognise this repo as a source of pkgbuilds just like the AUR.
...
This allows doing `upm -S ./foo` where `foo` is the name of a package (not path) under the
current directory. this means if you have a bunch of pkgbuilds in a directory that depend on
each other you can build one with `upm -S ./foo` and upm will solve and build the dependencies
across pkgbuilds.

Previously `upm -U` could be used to build a pkgbuild in the current directory. This has been
renamed to `upm -B <dirs>...` allowing you to specify multiple pkgbuilds to build at once.

See `upm.conf(5)` for more information on this.
...
`upm foo` has always been there for interactive search and install. There is now `--interactive`
which works for other operactions.

- `upm foo` is an alias for `upm -S --interactive foo`
- `upm -R --interactive foo` can be used for an interactive remove.
- `upm -Ss/-Qs --interactive foo` will give an interactive prompt then print the chosen packages so they can be piped to other commands.

### Provides

Provide searching is now better and is now enabled in the default upm.conf. You probably
want to uncomment this option if you have an existing upm.conf.

### Contributors

Thanks to every one who has contributed code and translations for upm.
...
- No confirm pacman install after final upm confirmation
...
- Fix no sudo when running upm -Sc
- Fix --redownload=yes
...
## UPM v1.11.2 (2022-11-05)

Rebuild for openssl 3

## UPM v1.11.1 (2022-07-06)
...
- Fix typo in upm.conf man page #709
...
## UPM v1.9.2 (2021-12-09)

## Fixed

- Fix upm not actually waiting on children properly e336648
...
## UPM v1.9.1 (2021-11-26)

### Fixed

- Make upm -G return a correct exit code #601
...
## UPM v1.7.1 (2021-06-03)

### Fixed

- Fix panic in provider menu 73ecc7

## UPM v1.7.0 (2021-05-31)
...
- UPM now sets the user agent for aur requests 841ebb5
...
## UPM v1.1.4 (2020-12-21)

### Fixed

- Remove nonexistent topdown option from `upm.conf(5)` #124
...
- Warn instead of error on unknown values in upm.conf 0e4e618

## UPM v1.1.2 (2020-11-18)
...
- Remove dashes from InstallDebug in upm.conf.5 #86
...
- Fix `upm <target>` trying to install all packages when input is empty ba00b9f
...
- Add `UPM_CONF` to configure upm's config file location eb9e1b1
...
## UPM v1.0.0 (2020-10-28) - changes from yay
...
- Auto detection for pacman-git. UPM will build against pacman-git's API
...
- Packages are now cloned to ~/.cache/upm/clone by default
